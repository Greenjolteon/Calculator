...
/*
Equations that dont work (returned value, what it should return):
Doesnt handle negative terms well
3+x+2 (0, -5)
*/
...
const opposites = {'^': '√','√': '^','sin': 'arcsin','cos': 'arccos','sinh': 'arcsinh','cosh': 'arccosh','tanh': 'arctanh','log': '10**','ln': 'exp','*': '/','/': '*','-': '+','+': '-',};
const associative = {'^': 0,'√': 1,'sin': 1,'cos': 1,'tan': 1,'sinh': 1,'cosh': 1,'tanh': 1,'log': 1,'ln': 1,'!': 1,'*': 1,'/': 1,'+': 1,'-': 1,};


function graph(h) {
	let array = h.match(/([0-9.]+|[+\-*^/()√!]|x|log|ln|sin|cos|tan|sinh|cosh|tanh|π|e)/g);
	console.log(arrayPrep(array));
	let narray;
	for (let i = 0; i < array.length; i++) {
		if (array[i].includes('x')) {
			if (i == 0) {
				if (array[i + 1] == '+') {
					narray = array.splice(i, 2);
				} else {
					narray = array.splice(i, 2);
				}
			} else {
				narray = array.splice(i - 1, 2)
			}
		}
	}
	console.log(array.length);
	if (narray.length == 1) {
		narray = termSep(narray);
		array = ['@', '@', '@'];
	}
	console.log(";Array: " + array);
	console.log("nArray: " + narray);
	if (array.length > 1 && !array.includes('@')) {
		narray.push(equate(array.join('')));
	} else {
		if (narray[0].includes('x')) {
			narray.push(parseInt(array[0]));
		} else if (narray[narray.length - 1].includes('x')) {
			narray.unshift(parseInt(array[0]));
		}
	}
	console.log("nArray 2: " + narray);
	narray = solver(narray);
	if (narray[0] == 'x') {
		return floatnumCorrect(solution);
	} else {
		console.log("Final narray error: " + narray);
		console.log("Final solution error: " + solution);
		termSep(narray);
		return "ERROR";
	}
}
...
function termSep(array) {
	array = array[0].match(/([0-9.]+|[+\-*^/()√!]|x|log|ln|sin|cos|tan|sinh|cosh|tanh|π|e)/g);
...
let eq = "x*25";
console.log("If " + eq + "=0, x=" + graph(eq));
